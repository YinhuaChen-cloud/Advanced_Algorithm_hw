...
def isQualify(points):
    ...
    for start in range(np):
        ...
        # print("greedy_len/optimal_len = " + str(greedy_len/optimal_len))
        assert(is_float_equal(greedy_len, optimal_len) or greedy_len >= optimal_len)
        if (greedy_len/optimal_len) > 1.5:
            # 打开文件，如果文件不存在则创建一个新文件，写入模式为追加模式
            global result_file
            result_file = open("result2-1-2.txt", "a")
            # 写入内容到文件
            result_file.write(str(points) + " ratio: " + str(greedy_len/optimal_len) + " greedy_path: " + str(greedy_path) + " optimal_path: " + str(optimal_path) + "\n")
            # 关闭文件
            result_file.close()
        ...
        for i in range(nm):
            ...
            for mutate_n in range(np):
                ...
                for start in range(np):
                    ...
                    assert(is_float_equal(greedy_len, optimal_len) or greedy_len >= optimal_len)
                    # 选择出最多起始点能跑出最优解的一个图
                    if (greedy_len/optimal_len) > greedy_count:
                        greedy_count = (greedy_len/optimal_len)
            ...
            # 用选择出的图替换掉当前图
            optimal_path, optimal_len = getOptimal(maps[i])
            greedy_count = -1
            for start in range(np):
                greedy_path, greedy_len = tsp_greedy(maps[i], start)
                assert(is_float_equal(greedy_len, optimal_len) or greedy_len >= optimal_len)
                # 选择出最多起始点能跑出最优解的一个图
                if (greedy_len/optimal_len) > greedy_count:
                    greedy_count = (greedy_len/optimal_len)

            if greedy_count < max_greedy_count:
                maps[i] = max_greedy_points
                print("greedy_len/optimal_len(substitute) = " + str(max_greedy_count))
            else:
                print("greedy_len/optimal_len(no substitute) = " + str(greedy_count))
