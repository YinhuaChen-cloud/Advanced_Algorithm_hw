...
def isQualify(points):
    ...
    # 如果所有点为起始点都能跑出最优解，那么把这个图写入 results.txt
    if count == 0:
        # 打开文件，如果文件不存在则创建一个新文件，写入模式为追加模式
        global result_file
        result_file = open("result1-3.txt", "a")
        # 写入内容到文件
        result_file.write("points: " + str(points) + "\n")
        optimal_path = optimal_path + optimal_path[0]
        result_file.write("optimal_path: " + str(optimal_path) + "\n")
        # 关闭文件
        result_file.close()
